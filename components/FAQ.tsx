        ...
        <section id="faq" className="py-24 bg-white/[0.02]">
            <div className="max-w-3xl mx-auto px-6">
                <div className="text-center mb-16">
                    ...
                    <p className="text-lg text-slate-600">Everything you need to know about the ETH Research Village.</p>
                </div>

                <div className="space-y-4">
                    {FAQ_ITEMS.map((item, idx) => (
                        <div key={idx} className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden transition-all duration-300">
                            <button
                                onClick={() => setOpenIndex(openIndex === idx ? null : idx)}
                                className="w-full flex items-center justify-between p-6 text-left hover:bg-white/[0.05] transition-colors"
                            >
                                <span className={`font-bold text-lg transition-colors ${openIndex === idx ? 'text-[#13ec5b]' : 'text-slate-900'}`}>
                                    {item.question}
                                </span>
                                {openIndex === idx ? (
                            ...
                            >
                                <div className="p-6 pt-0 border-t border-black/5 mt-2">
